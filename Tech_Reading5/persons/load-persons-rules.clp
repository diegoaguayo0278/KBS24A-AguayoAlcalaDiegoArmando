...
(defrule my-rule3
   (person (gender female) (name ?x)) 
   => 
   (printout t ?x " is female" crlf))

;; Defining a rule for finding persons' partners and printing their names
(defrule my-rule4
   (person (partner ?p) (name ?n)) 
   => 
   (printout t ?p " is " ?n "'s partner" crlf))

;; Defining a rule for finding and printing facts that are female persons
(defrule my-rule5
     ?p <- (person (gender female)) 
     => 
     (printout t ?p " is female" crlf))

;; Defining a rule for finding male persons older than 20 years and printing their names
(defrule my-rule6
   (person (gender male) (age ?a&:(> ?a 20)) (name ?n)) 
   => 
   (printout t ?n " is a male older than 20 years" crlf))
